...
class BertModel(nn.Module):
    ...
    def __init__(self, BATCH_SIZE=256, PATH='test', lr=0.001, embedding_layer=100,
                 ...
                 test_data_path="./scicite-data/test.jsonl", bertmodel_name='allenai/scibert_scivocab_uncased'):
    ...
    def plot(self):  # plot the model performance
        train_results_plot(self.bertmodel_name, self.total_train_loss, self.total_valid_loss,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--MODEL_NAME', type=str, default='allenai/scibert_scivocab_uncased', help="the bert model you choose, options: ['bert-base-uncased', 'bert-large-uncased', 'allenai/scibert_scivocab_uncased']")
    ...
    model = BertModel(BATCH_SIZE=args.BATCH_SIZE, lr=args.INITIAL_LR, embedding_layer=args.EMBEDDING_LAYER, train_data_path=args.TRAIN_PATH, valid_data_path=args.VAL_PATH,
                      test_data_path=args.TEST_PATH, bertmodel_name=args.MODEL_NAME)

    model.train(patient=args.EARLY_STOPPING_PATIENT, N_EPOCHS=args.N_EPOCHS, save_best_model=args.SAVE_BEST_MODEL, lradj=args.lradj)
    model.plot()
    model.test()